...
{
    [Validator("KEY005", Status.Error, Description = "Sublayout Datasource Location field is required.", Name = "Sublayouts: Datasource Location required")]
    public class DatasourceLocationRequired : UserConfigurableValidator
    {
            ...
            XDocument scprojDocument)
        {
            ...
            foreach (var item in projectItems)
            {
                    ...
                    x => item.Value.Item.SitecoreItemPath.StartsWith(x, StringComparison.InvariantCultureIgnoreCase)))
                {
                    ...
                    var fld = item.Value.ParsedItem.Fields["Datasource Location"];
                    ...
                    if (String.IsNullOrEmpty(fld) && !TemplateNamesToIgnore().Contains(item.Value.Item.TemplateName))
                    {
                        ...
                        {
                            Message =
                                string.Format("The 'Datasource Location' is required: {0}",
        ...
        private List<string> TemplateNamesToIgnore()
        {
